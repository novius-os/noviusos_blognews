<?php

// Generated on 02/12/2013 17:53:33

// 66 out of 66 messages are translated (100%).
// 285 out of 285 words are translated (100%).

return array(
    #: classes/model/post.model.php:157
    'Use post title' => 'Usar li titul del post.',

    #: classes/model/post.model.php:191
    'Use post summary' => 'Usar li summarium del post',

    #: classes/model/category.model.php:131
    'Use category title' => 'Usar li titul del categorie',

    #: classes/model/tag.model.php:47
    'Use tag label' => 'Usar li tag',

    #: views/admin/permissions/post_and_category.php:7
    'Can add, edit, delete and publish posts' => 'It permisse addir, modificar e deleter posts',

    #: views/admin/permissions/post_and_category.php:14
    'Can add, edit and delete unpublished posts only' => 'It permisse addir, modificar e deleter posts ne publicat solmen',

    #: views/admin/permissions/post_and_category.php:21
    'Can add, edit and delete categories' => 'It permisse addir, modificar e deleter categories',

    #. Other authors' posts
    #: views/admin/permissions/other_authors.php:9
    'Can edit and delete' => 'It permisse modificar e deleter.',

    #. Other authors' posts
    #: views/admin/permissions/other_authors.php:19
    'Can visualise only' => 'It permisse solmen exhibir',

    #. Other authors' posts
    #: views/admin/permissions/other_authors.php:29
    'Can access her/his own posts only' => 'Posse accesser solmen su propri posts ',

    #: config/permissions.config.php:11
    'Permissions for this application' => 'Permissions por ti application.',

    #: config/permissions.config.php:18
    'Other authors’ posts' => 'Posts de altri autors',

    #: config/controller/admin/inspector/author.config.php:22
    #: config/controller/admin/inspector/author.config.php:26
    'Authors' => 'Autors',

    #: config/controller/admin/inspector/tag.config.php:19
    #: config/controller/admin/post.config.php:61
    #: config/controller/admin/post.config.php:159
    #: config/controller/admin/post.config.php:203
    #: config/common/tag.config.php:16
    'Tags' => 'Tags',

    #: config/controller/admin/inspector/date.config.php:18
    'Created on' => 'Creat li',

    #: config/controller/admin/inspector/category.config.php:19
    'Root' => 'Radica',

    #: config/controller/admin/enhancer.config.php:14
    'Posts per page:' => 'Posts per pagine:',

    #: config/controller/admin/enhancer.config.php:20
    'Category:' => 'Categorie',

    #: config/controller/admin/enhancer.config.php:32
    'Leave blank to select all categories' => 'Clicar nequo por selecter omni categories',

    #: config/controller/admin/enhancer.config.php:35
    'Clickable post titles' => 'Titul del posts clicabil',

    #: config/controller/admin/post.config.php:23
    'Add a post' => 'Addir un post',

    #: config/controller/admin/post.config.php:24
    'Translate a post' => 'Traducter un post',

    #: config/controller/admin/post.config.php:39
    #: config/controller/admin/post.config.php:115
    'Content' => 'Contenete',

    #. user_fullname is not a real field in the database
    #: config/controller/admin/post.config.php:58
    #: config/controller/admin/post.config.php:212
    #: config/controller/admin/category.config.php:30
    'Properties' => 'Proprietas',

    #: config/controller/admin/post.config.php:59
    'URL (post address)' => 'URL (adresse del post)',

    #: config/controller/admin/post.config.php:60
    #: config/controller/admin/post.config.php:208
    #: config/common/category.config.php:24
    'Categories' => 'Categories',

    #: config/controller/admin/post.config.php:75
    #: config/controller/admin/category.config.php:52
    #: config/common/post.config.php:44
    'Title' => 'Titul',

    #: config/controller/admin/post.config.php:85
    'Summary' => 'Summarium',

    #: config/controller/admin/post.config.php:94
    'Change the author’s name (alias):' => 'Changear li nomine del autor (alias):',

    #: config/controller/admin/post.config.php:100
    #: config/controller/admin/category.config.php:62
    'URL:' => 'URL:',

    #: config/controller/admin/post.config.php:108
    'Author:' => 'Autor:',

    #: config/controller/admin/post.config.php:127
    'Thumbnail' => 'Viniette',

    #: config/controller/admin/post.config.php:132
    'Created on:' => 'Creat li:',

    #: config/controller/admin/post.config.php:143
    'Never read' => 'Nequande leet',

    #: config/controller/admin/post.config.php:144
    'Read once' => 'Leet un vez',

    #: config/controller/admin/post.config.php:145
    'Read {{nb}} times' => 'Leet {{nb}} vezes',

    #: config/controller/admin/category.config.php:19
    'Add a category' => 'Addir un categorie',

    #: config/controller/admin/category.config.php:20
    'Translate a category' => 'Traducter un categorie',

    #: config/controller/admin/category.config.php:71
    'Location:' => 'Localisation:',

    #: config/config.php:62
    'Blog' => 'Blog',

    #. Crud
    #: config/common/tag.config.php:22
    'The tag has been deleted.' => 'Li tag ha esset deletet.',

    #. General errors
    #: config/common/tag.config.php:25
    'This tag doesn’t exist any more. It has been deleted.' => 'Ti tag ne existe ancor. It ha esset deletet.',

    #: config/common/tag.config.php:26
    'We cannot find this tag.' => 'Noi ne posse truvar ti tag.',

    #. Deletion popup
    #: config/common/tag.config.php:29
    'Deleting the tag ‘{{title}}’' => 'Delete li tag ‘{{title}}’',

    #. Delete action's labels
    #: config/common/tag.config.php:32
    'Yes, delete this tag' => 'Yes, delete ti tag',

    #: config/common/post.config.php:48
    'Author' => 'Autor',

    #: config/common/post.config.php:55
    'Date' => 'Date',

    #. Crud
    #: config/common/category.config.php:29
    'There you go, the category has been added.' => 'Li categorie ha esset addit.',

    #: config/common/category.config.php:30
    'The category has been deleted.' => 'Li categorie ha esset deletet.',

    #. General errors
    #: config/common/category.config.php:33
    'This category doesn’t exist any more. It has been deleted.' => 'Ti categorie ne existe ancor. It ha esset deletet.',

    #: config/common/category.config.php:34
    'We cannot find this category.' => 'Noi ne posse truvar ti categorie.',

    #. Blank slate
    #: config/common/category.config.php:37
    'We’re afraid this category cannot be added to {{context}} because its <a>parent</a> is not available in this context yet.' => 'Desfortunatli, ti categorie ne posse esser addit a {{context}} pro que su <a>parent</a> ne es ancor disponibil in ti context.',

    #: config/common/category.config.php:38
    'We’re afraid this category cannot be translated into {{language}} because its <a>parent</a> is not available in this language yet.' => 'Desfortunatli, ti categorie ne posse esser traductet in {{language}} pro que su <a>parent</a> ne es ancor disponibil in ti context.',

    #. Deletion popup
    #: config/common/category.config.php:41
    'Deleting the category ‘{{title}}’' => 'Deleter li categorie ‘{{title}}’',

    #. Delete action's labels
    #: config/common/category.config.php:44
    'Yes, delete this category' => 'Yes, delete ti categorie',

    #: config/common/category.config.php:45
    'Yes, delete these {{count}} categories' => 'Yes, delete ti {{count}} categories',

    #: config/common/category.config.php:47
    '1 category' => '1 categorie',

    #: config/common/category.config.php:48
    '{{count}} categories' => '{{count}} categories',

    #. Keep only if the model has the behaviour Contextable
    #: config/common/category.config.php:51
    'This category exists in <strong>{{context_count}} contexts</strong>.' => 'Ti categorie existe in <strong>{{context_count}} contextus</strong>.',

    #: config/common/category.config.php:52
    'This category exists in <strong>{{language_count}} languages</strong>.' => 'Ti categorie existe in <strong>{{language_count}} lingues</strong>.',

    #. Keep only if the model has the behaviours Contextable + Tree
    #: config/common/category.config.php:55
    'This category exists in <strong>{{context_count}} contexts</strong> and has <strong>{{children_count}} sub-categories</strong>.' => 'Ti categorie existe in <strong>{{context_count}} contexuts</strong> e have <strong>{{children_count}} sub-categories</strong>.',

    #: config/common/category.config.php:56
    'This category exists in <strong>{{context_count}} contexts</strong> and has <strong>one sub-category</strong>.' => 'Ti categorie existe in <strong>{{context_count}} contextus</strong> e have <strong>un sub-categorie</strong>.',

    #: config/common/category.config.php:57
    'This category exists in <strong>{{language_count}} languages</strong> and has <strong>{{children_count}} sub-categories</strong>.' => 'Ti categorie existe in <strong>{{language_count}} lingues</strong> e have <strong> {{children_count}} sub-categories</strong>.',

    #: config/common/category.config.php:58
    'This category exists in <strong>{{language_count}} languages</strong> and has <strong>one sub-category</strong>.' => 'Ti categorie existe in <strong>{{language_count}} lingues</strong> e have <strong>un sub-categorie</strong>.',

    #. Keep only if the model has the behaviour Tree
    #: config/common/category.config.php:61
    'This category has <strong>1 sub-category</strong>.' => 'Ti categorie have <strong>1 sub-categorie</strong>.',

    #: config/common/category.config.php:62
    'This category has <strong>{{children_count}} sub-categories</strong>.' => 'Ti categorie have <strong>{{children_count}} sub-categories</strong>.',

);
